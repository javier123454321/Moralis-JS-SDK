...
class InternalWeb3Provider extends EventEmitter {
  ...
  handleConnect(connectInfo) {
    this.emit(InternalWeb3Events.CONNECT, connectInfo);
  }

  // Handle Disconnect events fired from connectors
  handleDisconnect(error) {
    this.emit(InternalWeb3Events.DISCONNECT, error);
  }
  ...
  async deactivate() {
    ...
    if (this.connector) {
      if (this.connector.removeListener) {
        ...
        this.connector.removeListener(InternalWeb3Events.CONNECT, this.handleConnect);
        this.connector.removeListener(InternalWeb3Events.DISCONNECT, this.handleDisconnect);
      }
